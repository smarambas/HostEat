...
public class RateUserController {
	...
	public void rateUser(SessionBean session, UserBean userBean, EventBean eventBean, int rating) throws ClassNotFoundException, SQLException, NoRecordFoundException, IOException, ParseException, AlreadyRatedException {
		...
		if(session.getUserBean().getUserType().equalsIgnoreCase("HOST")) {
			...
			User guest = UserDAO.retrieveUserByUsername(session.getUserBean().getUsername());
